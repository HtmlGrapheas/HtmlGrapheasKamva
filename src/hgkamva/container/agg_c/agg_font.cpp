...
void cairo_font::show_text(
    cairo_t* cr, int x, int y, const litehtml::tchar_t* str)
{
  lock();
...
void cairo_font::split_text(
    const litehtml::tchar_t* src, text_chunk::vector& chunks)
{
  wchar_t* str;
#ifdef LITEHTML_UTF8
  ...
  while (cch > 0) {
    ...
    long cchActual;
    if (m_font_link) {
      hr = m_font_link->GetStrCodePages(
          str, cch, m_font_code_pages, &dwActualCodePages, &cchActual);
    } else {
      hr = S_FALSE;
    }
    ...
    int sz = WideCharToMultiByte(
                 CP_UTF8, 0, str, cchActual, chk->text, 0, NULL, NULL)
        + 1;
    chk->text = new CHAR[sz];
    sz = WideCharToMultiByte(
        CP_UTF8, 0, str, cchActual, chk->text, sz, NULL, NULL);
    chk->text[sz] = 0;
    chk->font = NULL;

    if (!(dwActualCodePages & m_font_code_pages)) {
      for (linked_font::vector::iterator i = m_linked_fonts.begin();
           i != m_linked_fonts.end(); i++) {
  ...
  if (hr != S_OK) {
    ...
    int sz =
        WideCharToMultiByte(CP_UTF8, 0, str, -1, chk->text, 0, NULL, NULL) + 1;
    chk->text = new CHAR[sz];

...
cairo_container::cairo_container(void)
{
  ...
  m_font_link = NULL;
  CoCreateInstance(CLSID_CMultiLanguage, NULL, CLSCTX_ALL, IID_IMLangFontLink2,
      (void**) &m_font_link);
...
litehtml::uint_ptr cairo_container::create_font(
    ...
    litehtml::font_metrics* fm)
{
  ...
  cairo_font* fnt = new cairo_font(m_font_link, fnt_name.c_str(), size, weight,
...
int cairo_container::text_width(
...
void cairo_container::draw_text(litehtml::uint_ptr hdc,
...
void cairo_container::draw_list_marker(
    litehtml::uint_ptr hdc, const litehtml::list_marker& marker)
{
  if (!marker.image.empty()) {
    ...
    if (img_i != m_images.end()) {
      if (img_i->second) {
        draw_txdib((cairo_t*) hdc, img_i->second.get(), marker.pos.x,
...
void cairo_container::load_image(const litehtml::tchar_t* src,
...
void cairo_container::get_image_size(const litehtml::tchar_t* src,
    const litehtml::tchar_t* baseurl,
    litehtml::size& sz)
{
  std::wstring url;
...
void cairo_container::draw_image(litehtml::uint_ptr hdc,
...
void cairo_container::draw_background(
    litehtml::uint_ptr hdc, const litehtml::background_paint& bg)
{
  ...
  if (img_i != m_images.end() && img_i->second) {
    ...
    image_ptr new_img;
    if (bg.image_size.width != bgbmp->getWidth()
        || bg.image_size.height != bgbmp->getHeight()) {
    ...
    cairo_surface_t* img = cairo_image_surface_create_for_data(
        (unsigned char*) bgbmp->getBits(), CAIRO_FORMAT_ARGB32,
        bgbmp->getWidth(), bgbmp->getHeight(), bgbmp->getWidth() * 4);
    ...
    switch (bg.repeat) {
      case litehtml::background_repeat_no_repeat:
        draw_txdib(cr, bgbmp.get(), bg.position_x, bg.position_y,
      ...
      case litehtml::background_repeat_repeat_x:
        ...
        cairo_rectangle(cr, bg.clip_box.left(), bg.position_y,
            bg.clip_box.width, bgbmp->getHeight());
        cairo_fill(cr);
        break;

      case litehtml::background_repeat_repeat_y:
        cairo_set_source(cr, pattern);
        cairo_rectangle(cr, bg.position_x, bg.clip_box.top(), bgbmp->getWidth(),
            bg.clip_box.height);
        cairo_fill(cr);
        break;
...
bool cairo_container::add_path_arc(cairo_t* cr,
    double x,
    double y,
    double rx,
    double ry,
...
void cairo_container::draw_borders(litehtml::uint_ptr hdc,
    ...
    bool root)
{
  ...
  if (bdr_right) {
    ...
    if (r_top) {
      ...
      double start_angle = end_angle
          - M_PI / 2.0 / ((double) bdr_top / (double) bdr_right + 0.5);

      if (!add_path_arc(cr, draw_pos.right() - r_top, draw_pos.top() + r_top,
    ...
    if (r_bottom) {
      ...
      double end_angle = start_angle
          + M_PI / 2.0 / ((double) bdr_bottom / (double) bdr_right + 0.5);

      if (!add_path_arc(cr, draw_pos.right() - r_bottom,
              draw_pos.bottom() - r_bottom, r_bottom, r_bottom, start_angle,
              end_angle, false)) {
        cairo_line_to(cr, draw_pos.right(), draw_pos.bottom());
      }

      if (!add_path_arc(cr, draw_pos.right() - r_bottom,
              draw_pos.bottom() - r_bottom, r_bottom - bdr_right,
              r_bottom - bdr_right + (bdr_right - bdr_bottom), end_angle,
              start_angle, true)) {
        cairo_line_to(
            cr, draw_pos.right() - bdr_right, draw_pos.bottom() - bdr_bottom);
      }
    } else {
      cairo_line_to(cr, draw_pos.right(), draw_pos.bottom());
      cairo_line_to(
  ...
  if (bdr_bottom) {
    ...
    if (r_left) {
      ...
      double end_angle = start_angle
          + M_PI / 2.0 / ((double) bdr_left / (double) bdr_bottom + 0.5);

      if (!add_path_arc(cr, draw_pos.left() + r_left,
              draw_pos.bottom() - r_left,
              r_left - bdr_bottom + (bdr_bottom - bdr_left),
              r_left - bdr_bottom, start_angle, end_angle, false)) {
        cairo_move_to(
            cr, draw_pos.left() + bdr_left, draw_pos.bottom() - bdr_bottom);
      }

      if (!add_path_arc(cr, draw_pos.left() + r_left,
              draw_pos.bottom() - r_left, r_left, r_left, end_angle,
              start_angle, true)) {
    ...
    if (r_right) {
      ...
      double start_angle = end_angle
          - M_PI / 2.0 / ((double) bdr_right / (double) bdr_bottom + 0.5);

      if (!add_path_arc(cr, draw_pos.right() - r_right,
              draw_pos.bottom() - r_right, r_right, r_right, end_angle,
              start_angle, true)) {
        cairo_line_to(cr, draw_pos.right(), draw_pos.bottom());
      }
  ...
  if (bdr_top) {
    ...
    if (r_left) {
      ...
      double start_angle =
          end_angle - M_PI / 2.0 / ((double) bdr_left / (double) bdr_top + 0.5);

      if (!add_path_arc(cr, draw_pos.left() + r_left, draw_pos.top() + r_left,
    ...
    if (r_right) {
      ...
      double end_angle = start_angle
          + M_PI / 2.0 / ((double) bdr_right / (double) bdr_top + 0.5);
      ...
      if (!add_path_arc(cr, draw_pos.right() - r_right,
              draw_pos.top() + r_right, r_right, r_right, end_angle,
              start_angle, true)) {
        cairo_line_to(cr, draw_pos.right(), draw_pos.top());
      }
    } else {
  ...
  if (bdr_left) {
    ...
    if (r_top) {
      ...
      double end_angle = start_angle
          + M_PI / 2.0 / ((double) bdr_top / (double) bdr_left + 0.5);

      if (!add_path_arc(cr, draw_pos.left() + r_top, draw_pos.top() + r_top,
    ...
    if (r_bottom) {
      ...
      double start_angle = end_angle
          - M_PI / 2.0 / ((double) bdr_bottom / (double) bdr_left + 0.5);

      if (!add_path_arc(cr, draw_pos.left() + r_bottom,
              draw_pos.bottom() - r_bottom, r_bottom, r_bottom, end_angle,
              start_angle, true)) {
        cairo_line_to(cr, draw_pos.left(), draw_pos.bottom());
      }

      if (!add_path_arc(cr, draw_pos.left() + r_bottom,
              draw_pos.bottom() - r_bottom, r_bottom - bdr_left,
              r_bottom - bdr_left + (bdr_left - bdr_bottom), start_angle,
              end_angle, false)) {
        cairo_line_to(
            cr, draw_pos.left() + bdr_left, draw_pos.bottom() - bdr_bottom);
      }
...
void cairo_container::set_clip(const litehtml::position& pos,
...
void cairo_container::draw_ellipse(cairo_t* cr,
    int x,
    int y,
    int width,
    int height,
...
void cairo_container::fill_ellipse(cairo_t* cr,
    int x,
    int y,
    int width,
    int height,
...
void cairo_container::draw_txdib(
    cairo_t* cr, CTxDIB* bmp, int x, int y, int cx, int cy)
{
  cairo_save(cr);
  ...
  if (cx != bmp->getWidth() || cy != bmp->getHeight()) {
    ...
    img = cairo_image_surface_create_for_data((unsigned char*) rbmp.getBits(),
        CAIRO_FORMAT_ARGB32, rbmp.getWidth(), rbmp.getHeight(),
        rbmp.getWidth() * 4);
    cairo_matrix_translate(&flib_m, 0, -rbmp.getHeight());
    cairo_matrix_translate(&flib_m, x, -y);
  } else {
    img = cairo_image_surface_create_for_data((unsigned char*) bmp->getBits(),
...
void cairo_container::rounded_rectangle(cairo_t* cr,
    const litehtml::position& pos,
    const litehtml::border_radiuses& radius)
{
  ...
  if (radius.top_right_x) {
    cairo_arc(cr, pos.right() - radius.top_right_x,
        pos.top() + radius.top_right_x, radius.top_right_x, M_PI * 3.0 / 2.0,
        2.0 * M_PI);
  }
  ...
  if (radius.bottom_right_x) {
    cairo_arc(cr, pos.right() - radius.bottom_right_x,
        pos.bottom() - radius.bottom_right_x, radius.bottom_right_x, 0,
        M_PI / 2.0);
  }
  ...
  if (radius.bottom_left_x) {
    cairo_arc(cr, pos.left() + radius.bottom_left_x,
        pos.bottom() - radius.bottom_left_x, radius.bottom_left_x, M_PI / 2.0,
        M_PI);
  }
}
...
std::shared_ptr<litehtml::element> cairo_container::create_element(
...
void cairo_container::get_language(
...
void cairo_container::make_url_utf8(
    const char* url, const char* basepath, std::wstring& out)
{
...
void cairo_container::link(const std::shared_ptr<litehtml::document>& doc,
    const litehtml::element::ptr& el)
{
}

litehtml::tstring cairo_container::resolve_color(
    const litehtml::tstring& color) const
{
  ...
  static custom_color colors[] = {{_t("ActiveBorder"), COLOR_ACTIVEBORDER},
      {_t("ActiveCaption"), COLOR_ACTIVECAPTION},
      {_t("AppWorkspace"), COLOR_APPWORKSPACE},
      {_t("Background"), COLOR_BACKGROUND}, {_t("ButtonFace"), COLOR_BTNFACE},
      {_t("ButtonHighlight"), COLOR_BTNHIGHLIGHT},
      {_t("ButtonShadow"), COLOR_BTNSHADOW}, {_t("ButtonText"), COLOR_BTNTEXT},
      {_t("CaptionText"), COLOR_CAPTIONTEXT}, {_t("GrayText"), COLOR_GRAYTEXT},
      ...
      {_t("InfoBackground"), COLOR_INFOBK}, {_t("InfoText"), COLOR_INFOTEXT},
      {_t("Menu"), COLOR_MENU}, {_t("MenuText"), COLOR_MENUTEXT},
      ...
      {_t("ThreeDShadow"), COLOR_3DSHADOW}, {_t("Window"), COLOR_WINDOW},
...
#ifdef LITEHTML_UTF8
      StringCchPrintfA(str_clr, 20, "#%02X%02X%02X", GetRValue(rgb_color),
          GetGValue(rgb_color), GetBValue(rgb_color));
#else
      StringCchPrintf(str_clr, 20, L"#%02X%02X%02X", GetRValue(rgb_color),
          GetGValue(rgb_color), GetBValue(rgb_color));
#endif  // LITEHTML_UTF8

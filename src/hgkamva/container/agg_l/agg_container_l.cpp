...
litehtml::uint_ptr container_linux::create_font(
...
int container_linux::text_width(
...
void container_linux::draw_list_marker(
    litehtml::uint_ptr hdc, const litehtml::list_marker& marker)
{
  if (!marker.image.empty()) {
    ...
    if (img_i != m_images.end()) {
      if (img_i->second) {
        draw_txdib((cairo_t*) hdc, img_i->second, marker.pos.x, marker.pos.y,
...
void container_linux::draw_background(
    litehtml::uint_ptr hdc, const litehtml::background_paint& bg)
{
  ...
  if (img_i != m_images.end() && img_i->second) {
    ...
    if (bg.image_size.width != bgbmp->get_width()
      ...
      new_img = bgbmp->scale_simple(
          bg.image_size.width, bg.image_size.height, Gdk::INTERP_BILINEAR);
      bgbmp = new_img;
    }
    ...
    switch (bg.repeat) {
      case litehtml::background_repeat_no_repeat:
        draw_pixbuf(cr, bgbmp, bg.position_x, bg.position_y, bgbmp->get_width(),
            bgbmp->get_height());
        break;
...
void container_linux::add_path_arc(cairo_t* cr,
    double x,
    double y,
    double rx,
    double ry,
    ...
    bool root)
{
  ...
  if (bdr_right) {
    ...
    if (r_top) {
      ...
      add_path_arc(cr, draw_pos.right() - r_top, draw_pos.top() + r_top, r_top,
          r_top, start_angle, end_angle, false);
    } else {
    ...
    if (r_bottom) {
      ...
      add_path_arc(cr, draw_pos.right() - r_bottom,
  ...
  if (bdr_left) {
    ...
    if (r_top) {
      ...
      add_path_arc(cr, draw_pos.left() + r_top, draw_pos.top() + r_top, r_top,
          r_top, end_angle, start_angle, true);
    } else {
...
void container_linux::transform_text(
...
void container_linux::draw_ellipse(cairo_t* cr,
    int x,
    int y,
    int width,
    int height,
...
void container_linux::fill_ellipse(cairo_t* cr,
    int x,
    int y,
    int width,
    int height,
...
void container_linux::rounded_rectangle(cairo_t* cr,
    const litehtml::position& pos,
...
void container_linux::draw_pixbuf(cairo_t* cr,
    const Glib::RefPtr<Gdk::Pixbuf>& bmp,
    int x,
    int y,
    int cx,
    int cy)
...
cairo_surface_t* container_linux::surface_from_pixbuf(
    const Glib::RefPtr<Gdk::Pixbuf>& bmp)
{
...
void container_linux::get_language(
